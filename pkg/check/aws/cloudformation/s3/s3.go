...
func CheckS3BucketPublicReadAcl() ([]result.CheckResult, error) {
	...
	for logicalID, resource := range template.GetAllS3BucketResources() {
		if resource.AccessControl != nil && *resource.AccessControl == "PublicRead" {
			output = append(output, result.CheckResult{Result: result.FAIL, Severity: result.Medium, Message: fmt.Sprintf("S3 Bucket %s should not have a public-read acl", logicalID)})
		}
	}
...
func CheckS3ObjectVersioningRule() ([]result.CheckResult, error) {
	...
	for logicalID, resource := range template.GetAllS3BucketResources() {
		if resource.VersioningConfiguration != nil && resource.VersioningConfiguration.Status != "Enabled" {
			output = append(output, result.CheckResult{Result: result.FAIL, Severity: result.Medium, Message: fmt.Sprintf("S3 Bucket %s is required to have object versioning enabled", logicalID)})
		}
	}
